...
;;; being backed up is prepended to the backup file name, with each "/" replaced
;;; by a "!".  This is intended for cases where an absolute backup path is used.
;;; If OPTIONS contains `prepend-name' in addition to `full-path', then the file
;;; name is prepended rather than appended to the path component when forming
;;; the backup name.
;;;
...
;;; Change Log:
;;;
;;; 6/10/1998  Version 2.7
;;; Add optional N argument to file-newest-backup and  find-file-latest-backup.
;;;
;;; 3/23/1998  Version 2.6
;;; Have to quote the eval-after-load form.
;;;
;;; 2/22/1998  Version 2.5
...
;;;
;;; 2/22/1998  Version 2.4
;;; Add a new option: prepend-name, suggested by Dave Bakhash.
;;;
;;; 2/2/1998  Version 2.3
;;; Nuke byte-compiler-options.  Use eval-when-compile defvars to avoid
;;; compiler warnings.  Remove unused binding of non-num-bk-name from
;;; find-backup-file-name.
;;;
;;; 10/27/1997  Version 2.2
...
;;; `bv-length'.  Note, we continue to support older GNU Emacs and XEmacs
...
(eval-when-compile
  ...
  (if (not (fboundp 'msdos-long-file-names))
          ...
          (while (and (not bk-dir) alist)
            (if (or (eq (car (car alist)) t)
                ...
                (let* ((bd            (car (cdr (car alist))))
                       ...
                       (bd-noslash    (if (eq (aref bd-expn (1- (length
bd-expn))) ?/)
                                          (substring bd-expn 0 (1- (length
bd-expn)))
                                        bd-expn))
                       (options       (cdr (cdr (car alist))))
                       (ok-create     (and (memq 'ok-create     options) t))
                       (search-upward (and (memq 'search-upward options)
t)))
                  (if bd-expn
                      (cond ((or (file-directory-p bd-expn)
                                 (and ok-create
                                      (not (file-exists-p bd-expn))
                                      (bkup-try-making-directory
bd-noslash)))
...
(defun bkup-existing-backup-files (fn)
  ...
  (let* ((efn (expand-file-name fn))
         ...
         (possibilities (file-name-all-completions base-versions
backup-dir))
...
is specified with the prefix argument."
  (interactive (list (read-file-name (format "Find %slatest backup of file
(default %s): "
                                             (let* ((arg
...
(buffer-file-name)))
  ...
  (let ((backup (file-newest-backup file n)))
    (if backup
        (find-file backup)
      (message "no %s found for `%s'" (if (> n 1) "such backup" "backups")
file))))
